...
def ai_move():
    ...
    env.step(ai_action)  # Apply the AI's action to the environment
    state = get_game_state()  # Get the new game state
    return state

# Perform human player's move
def human_move(card: str):
    legal_actions = get_game_state()['legal_actions']
    # Validate and process the human player's move
    if card in legal_actions:
        env.step(card)  # Apply human's action to the environment
        state = get_game_state()  # Get the new game state
        return state
    else:
        return {"error": "Invalid move, card not in legal actions"}


def player_move(card:str):
    trajectories, payoffs = env.run_with_api(card)
